# -*- coding: utf-8 -*-
'''Simulation of online reward updated architectures where seed for random process are passed as parameter'''
...
def execute_simulation(nbRep, horizon, pol_index, process, tr):
    if scenario == 'bernoulli_low_var_random':
        config = Delayed_Conf_Random_poisson(1./80, 300, r_seed=rseed)
        env = MABEventDelayed(
        ...
        config = Delayed_DR_Conf_Random_poisson(1./150, 480, r_seed=rseed)
        env = MABEventDelayed(
